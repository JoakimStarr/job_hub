...
describe('RecommendationHistory - 推荐历史记录', () => {
  ...
  describe('saveRecommendationHistory', () => {
    it('应保存推荐记录并返回完整的记录对象', () => {
      ...
      getDb().prepare.mockImplementation((sql: string) => {
        ...
        if (sql.includes('INSERT INTO recommendation_history')) {
          ...
          s.get.mockReturnValue(savedRecord)
        }
        return s
      })

      const result = saveRecommendationHistory({ mode: 'analyze', jobId: 42, result: { score: 85, matchRate: 0.9 } })
    ...
    it('应保存所有提供的字段', () => {
      const fullRecord: RecommendationHistoryItem = {
        id: 2, user_id: 10, mode: 'chat', job_id: 5,
      ...
      getDb().prepare.mockImplementation((sql: string) => {
        ...
        if (sql.includes('INSERT INTO recommendation_history')) {
          ...
          s.get.mockReturnValue(fullRecord)
        }
        return s
      })

      const result = saveRecommendationHistory({
        userId: 10, mode: 'chat', jobId: 5,
    ...
    it('status默认应为success', () => {
      const defaultStatusRecord: RecommendationHistoryItem = {
        id: 3, mode: 'analyze', result: {}, status: 'success', created_at: new Date().toISOString(),
      }

      getDb().prepare.mockImplementation((sql: string) => {
        const s = { get: vi.fn(), run: vi.fn(), all: vi.fn() }
        if (sql.includes('INSERT INTO recommendation_history')) s.run.mockReturnValue({ lastInsertRowid: 3, changes: 1 })
        else if (sql.includes('SELECT * FROM recommendation_history WHERE id = ?')) s.get.mockReturnValue(defaultStatusRecord)
        return s
      })

      const result = saveRecommendationHistory({ mode: 'analyze', result: {} })
      expect(result.status).toBe('success')
    })

    it('应支持error状态', () => {
      const errorRecord: RecommendationHistoryItem = {
        id: 4, mode: 'ai_analysis', result: { error: 'API timeout' }, status: 'error', created_at: new Date().toISOString(),
      }

      getDb().prepare.mockImplementation((sql: string) => {
        const s = { get: vi.fn(), run: vi.fn(), all: vi.fn() }
        if (sql.includes('INSERT INTO recommendation_history')) s.run.mockReturnValue({ lastInsertRowid: 4, changes: 1 })
        else if (sql.includes('SELECT * FROM recommendation_history WHERE id = ?')) s.get.mockReturnValue(errorRecord)
        return s
      })

      const result = saveRecommendationHistory({ mode: 'ai_analysis', result: { error: 'API timeout' }, status: 'error' })
    ...
    it('profile_summary超过500字符应截断', () => {
      ...
      const truncatedRecord: RecommendationHistoryItem = {
        id: 5, mode: 'analyze', profile_summary: longSummary.slice(0, 500),
        result: {}, status: 'success', created_at: new Date().toISOString(),
      }

      getDb().prepare.mockImplementation((sql: string) => {
        const s = { get: vi.fn(), run: vi.fn(), all: vi.fn() }
        if (sql.includes('INSERT INTO recommendation_history')) s.run.mockReturnValue({ lastInsertRowid: 5, changes: 1 })
        else if (sql.includes('SELECT * FROM recommendation_history WHERE id = ?')) s.get.mockReturnValue(truncatedRecord)
        return s
      })

      const result = saveRecommendationHistory({ mode: 'analyze', profileSummary: longSummary, result: {} })
    ...
    it('prompt超过500字符应截断', () => {
      ...
      const truncatedRecord: RecommendationHistoryItem = {
        id: 6, mode: 'chat', prompt: longPrompt.slice(0, 500), result: {}, status: 'success', created_at: new Date().toISOString(),
      }

      getDb().prepare.mockImplementation((sql: string) => {
        const s = { get: vi.fn(), run: vi.fn(), all: vi.fn() }
        if (sql.includes('INSERT INTO recommendation_history')) s.run.mockReturnValue({ lastInsertRowid: 6, changes: 1 })
        else if (sql.includes('SELECT * FROM recommendation_history WHERE id = ?')) s.get.mockReturnValue(truncatedRecord)
        return s
      })

      const result = saveRecommendationHistory({ mode: 'chat', prompt: longPrompt, result: {} })
    ...
    it('result应序列化为JSON字符串存储', () => {
      ...
      getDb().prepare.mockImplementation((sql: string) => {
        const s = { get: vi.fn(), run: vi.fn(), all: vi.fn() }
        if (sql.includes('INSERT INTO recommendation_history')) s.run.mockReturnValue({ lastInsertRowid: 7, changes: 1 })
        else s.get.mockReturnValue({ id: 7, mode: 'analyze', result: complexResult, status: 'success', created_at: new Date().toISOString() })
        return s
      })
  ...
  describe('getRecommendationHistory', () => {
    it('应返回历史记录列表', () => {
      ...
      getDb().prepare.mockImplementation((sql: string) => {
        const s = { get: vi.fn(), run: vi.fn(), all: vi.fn() }
        if (sql.includes('FROM recommendation_history WHERE 1=1')) s.all.mockReturnValue(mockRecords)
        return s
      })
    ...
    it('应支持userId过滤', () => {
      getDb().prepare.mockImplementation((sql: string) => {
        const s = { get: vi.fn(), run: vi.fn(), all: vi.fn() }
        if (sql.includes('user_id = ?')) s.all.mockReturnValue([{ id: 1, user_id: 5, mode: 'analyze', result: {}, status: 'success', created_at: '2024-01-01' }])
        return s
      })
    ...
    it('应支持mode过滤', () => {
      getDb().prepare.mockImplementation((sql: string) => {
        const s = { get: vi.fn(), run: vi.fn(), all: vi.fn() }
        if (sql.includes('mode = ?')) s.all.mockReturnValue([
    ...
    it('应支持jobId过滤', () => {
      getDb().prepare.mockImplementation((sql: string) => {
        const s = { get: vi.fn(), run: vi.fn(), all: vi.fn() }
        if (sql.includes('job_id = ?')) s.all.mockReturnValue([{ id: 1, job_id: 42, mode: 'ai_analysis', result: {}, status: 'success', created_at: '2024-01-01' }])
        return s
      })
    ...
    it('结果应按created_at降序排列', () => {
      getDb().prepare.mockImplementation((sql: string) => {
        const s = { get: vi.fn(), run: vi.fn(), all: vi.fn() }
        if (sql.includes('FROM recommendation_history WHERE 1=1')) s.all.mockReturnValue([])
        return s
      })

      getRecommendationHistory()
      const sqlCalls = getDb().prepare.mock.calls.map((c: [string]) => c[0])
  ...
  describe('去重逻辑', () => {
    it('相同参数的多次调用应产生不同ID的记录', () => {
      ...
      getDb().prepare.mockImplementation((sql: string) => {
        ...
        if (sql.includes('INSERT INTO recommendation_history')) {
          insertId++
          s.run.mockReturnValue({ lastInsertRowid: insertId, changes: 1 })
        } else {
          s.get.mockReturnValue({ id: insertId, mode: 'analyze', result: {}, status: 'success', created_at: new Date().toISOString() })
        }
        return s
      })
  ...
  describe('模式有效性验证', () => {
    it('应接受所有合法的mode值', () => {
      ...
      for (const mode of validModes) {
        ...
        getDb().prepare.mockImplementation((sql: string) => {
          const s = { get: vi.fn(), run: vi.fn(), all: vi.fn() }
          if (sql.includes('INSERT INTO recommendation_history')) s.run.mockReturnValue({ lastInsertRowid: record.id, changes: 1 })
          else s.get.mockReturnValue(record)
          return s
        })
